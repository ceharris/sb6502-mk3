	...
	; Time-of-day constants represented in BCD
	...
	; 32-bit unsigned integer. With a tick pulse of 100 Hz
	; the counter, it rolls over after ~497 days. 
timer_ticks:
		.res 4

	; These fields are used to store the components of the
	; chronometer, in BCD format
chrono_day:				; days (0..9999)
		.res 2
chrono_hour:				; hours (00..23)
		.res 1
chrono_minute:				; minutes (00..59)
		.res 1
...
@chrono:
		...
		sta chrono_second
		cmp #SECONDS_PER_MINUTE	; carry out?
		bne @done
		stz chrono_second

		; carry into minutes
		sec			; simulate carry from previous add
		lda chrono_minute
		adc #0
		sta chrono_minute
		cmp #MINUTES_PER_HOUR	; carry out?
		bne @done
		stz chrono_minute
		...
		adc #0
		sta chrono_hour
		cmp #HOURS_PER_DAY	; carry out?
		bne @done
		stz chrono_hour

		; carry into day
		sec			; simulate carry from previous add
		lda chrono_day+1	; lower two digits first
		adc #0
		sta chrono_day+1
		bcc @done	
		lda chrono_day		; now upper two digits
		adc #0
		sta chrono_day
@done:
		plx
